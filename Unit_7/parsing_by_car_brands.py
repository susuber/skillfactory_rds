...
class Parsing:
    ...
    def car_info(self, url):
        ...
        earchBtn = self.browser.find_elements_by_class_name('Link SpoilerLink SpoilerLink_type_default')
        for item in earchBtn:
        ...
        try:
            ...
            complectation_dict = set()
            try:
                table_title = soup.find_all('li', class_='ComplectationGroupsDesktop__item')
                for item in table_title:
                    complectation_dict.add(item.next_element.next_element.next_element.text)
            except:
                pass

            try:
                complect_link = soup.find('a', class_='Link SpoilerLink CardCatalogLink SpoilerLink_type_default')\
                                    .get('href')
            except:
                print(f'[ERROR] Не удалось найти ссылку на характеристики модели')
            else:
                self.browser.get(url=complect_link)
                soup = bs(self.browser.page_source, "lxml")
                time.sleep(0.1 * randint(10, 15))

                try:
                    num = 0
                    table_dt = soup.find('div', class_='catalog__details-group')\
                                   .find_all('dt', class_='list-values__label')
                    table_dd = soup.find('div', class_='catalog__details-group')\
                                   .find_all('dd', class_='list-values__value')
                    for iter in table_dt:
                        if iter.text == 'Количество дверей':
                            num_door = num
                        num += 1

                    number_of_doors = table_dd[num_door].text
                except:
                ...
                try:
                    num = 0
                    table_dt = soup.find_all('dt', class_='list-values__label')
                    table_dd = soup.find_all('dd', class_='list-values__value')
                    for iter in table_dt:
                        if iter.text == 'Мощность':
                            num_door = num
                        num += 1

                    engine_power = table_dd[num_door].text
                except:
                    engine_power = None
                ...
                try:
                    num = 0
                    for iter in table_dt:
                        if iter.text == 'Страна марки':
                            num_door = num
                        num += 1

                    vendor = table_dd[num_door].text
                except:
                    vendor = None
                    self.__print_error(name='vendor', url=url)

                try:
                    complect_link = soup.find_all(class_='link link_pseudo catalog-tab-item tabs__link link__control i-bem')[2].get('href')
                    self.browser.get(url=complect_link)
                    time.sleep(0.1 * randint(10, 15))
                    soup = bs(self.browser.page_source, "lxml")
                    table_title = soup.find_all('li', class_='catalog__package-list-i')
                    for item in table_title:
                        complectation_dict.add(item.next_element.next_element.next_element.text)
                except:
                    pass
                ...
                self.data_car.append(
                    {
                        ...
                        'car_url': url,
                        'color': color,
                        'complectation_dict': list(complectation_dict),
